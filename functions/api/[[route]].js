...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (resource === 'general-tests') {
      ...
      if (!gt1Check || Number(gt1Check.c) !== 25) {
        await DB.prepare('DELETE FROM general_tests WHERE test_num = 1').run();
        // Approved quantitative bank (R-01) — Question_ID kept in comments for traceability.
        // Verbal section (v1-v5) pending; will be appended once authored.
        const GT1_SEED = [
          // ── q1: الحساب ──
          {qnum:1,skill:'q1',text:'اشترى تاجر بضاعة بمبلغ 800 ريال، ثم باعها بربح مقداره 15% من سعر الشراء. فما سعر البيع؟',opt1:'900',opt2:'920',opt3:'940',opt4:'960',ans:1}, // QB-AR-001
          {qnum:2,skill:'q1',text:'متوسط أربعة أعداد هو 18. إذا كانت ثلاثة منها هي: 12، 20، 22، فما العدد الرابع؟',opt1:'16',opt2:'18',opt3:'20',opt4:'24',ans:1}, // QB-AR-002
          {qnum:3,skill:'q1',text:'إذا كان (3/5) مبلغ يساوي 240 ريالًا، فما قيمة المبلغ كاملًا؟',opt1:'360',opt2:'380',opt3:'400',opt4:'420',ans:2}, // QB-AR-003
          {qnum:4,skill:'q1',text:'قطع سائق مسافة 180 كم خلال 3 ساعات بسرعة ثابتة. فإذا حافظ على السرعة نفسها، فكم يقطع خلال ساعتين ونصف؟',opt1:'120',opt2:'140',opt3:'150',opt4:'160',ans:2}, // QB-AR-004
          {qnum:5,skill:'q1',text:'إذا كان العامل (أ) ينجز عملاً في 12 يومًا، والعامل (ب) ينجزه في 18 يومًا، فكم يومًا يحتاجان إذا عملا معًا بالمعدل نفسه؟',opt1:'6.2',opt2:'7.2',opt3:'8',opt4:'9',ans:1}, // QB-AR-005
          // ── q2: الجبر ──
          {qnum:6,skill:'q2',text:'إذا كان 5س − 15 = 35، فإن قيمة س تساوي:',opt1:'8',opt2:'9',opt3:'10',opt4:'11',ans:2}, // QB-AL-001
          {qnum:7,skill:'q2',text:'قيمة 3(2س + 4) − 2(س − 1) تساوي:',opt1:'4س + 10',opt2:'5س + 12',opt3:'4س + 14',opt4:'6س + 10',ans:2}, // QB-AL-002
          {qnum:8,skill:'q2',text:'إذا كان س : ص = 4 : 7 وكان س = 20، فإن قيمة ص هي:',opt1:'28',opt2:'30',opt3:'35',opt4:'40',ans:2}, // QB-AL-003
          {qnum:9,skill:'q2',text:'إذا كان 2^5 ÷ 2^2 فإن الناتج يساوي:',opt1:'4',opt2:'6',opt3:'8',opt4:'16',ans:2}, // QB-AL-004
          {qnum:10,skill:'q2',text:'يزيد عمر أحمد على عمر أخيه بـ 6 سنوات، ومجموع عمريهما 34 سنة. كم عمر أحمد؟',opt1:'14',opt2:'18',opt3:'20',opt4:'22',ans:2}, // QB-AL-005
          // ── q3: الهندسة ──
          {qnum:11,skill:'q3',text:'مستطيل طوله 14 سم وعرضه 9 سم. فما مساحته؟',opt1:'46',opt2:'92',opt3:'126',opt4:'138',ans:2}, // QB-GE-001
          {qnum:12,skill:'q3',text:'مثلث أطوال أضلاعه 8 سم، 11 سم، 13 سم. فما محيطه؟',opt1:'30',opt2:'31',opt3:'32',opt4:'33',ans:2}, // QB-GE-002
          {qnum:13,skill:'q3',text:'دائرة نصف قطرها 7 سم. إذا اعتُمد π = (22/7)، فما مساحة الدائرة؟',opt1:'144',opt2:'154',opt3:'164',opt4:'176',ans:1}, // QB-GE-003
          {qnum:14,skill:'q3',text:'مثلث قائم الزاوية طولا ضلعيه القائمين 9 سم و12 سم. فما طول الوتر؟',opt1:'13',opt2:'14',opt3:'15',opt4:'16',ans:2}, // QB-GE-004
          {qnum:15,skill:'q3',text:'مثلثان متشابهان، معامل التشابه بينهما (3/5). إذا كان طول ضلع في المثلث الأكبر يساوي 30 سم، فما طول الضلع المناظر له في المثلث الأصغر؟',opt1:'16',opt2:'18',opt3:'20',opt4:'24',ans:1}, // QB-GE-005
          // ── q4: المقارنات الكمية ──
          {qnum:16,skill:'q4',text:'الكمية (أ): 18 × 4 — الكمية (ب): 24 × 3',opt1:'الكمية (أ) أكبر',opt2:'الكمية (ب) أكبر',opt3:'الكميتان متساويتان',opt4:'لا يمكن تحديد العلاقة',ans:2}, // QB-QC-001
          {qnum:17,skill:'q4',text:'الكمية (أ): (5/8) — الكمية (ب): (3/4)',opt1:'الكمية (أ) أكبر',opt2:'الكمية (ب) أكبر',opt3:'الكميتان متساويتان',opt4:'لا يمكن تحديد العلاقة',ans:1}, // QB-QC-002
          {qnum:18,skill:'q4',text:'الكمية (أ): √144 — الكمية (ب): 11',opt1:'الكمية (أ) أكبر',opt2:'الكمية (ب) أكبر',opt3:'الكميتان متساويتان',opt4:'لا يمكن تحديد العلاقة',ans:0}, // QB-QC-003
          {qnum:19,skill:'q4',text:'إذا كان س > 0، فقارن بين: الكمية (أ): س^2 ، والكمية (ب): س',opt1:'الكمية (أ) أكبر',opt2:'الكمية (ب) أكبر',opt3:'الكميتان متساويتان',opt4:'لا يمكن تحديد العلاقة',ans:3}, // QB-QC-004
          {qnum:20,skill:'q4',text:'الكمية (أ): مساحة مربع طول ضلعه 10 سم. الكمية (ب): مساحة مستطيل طوله 20 سم وعرضه 5 سم.',opt1:'الكمية (أ) أكبر',opt2:'الكمية (ب) أكبر',opt3:'الكميتان متساويتان',opt4:'لا يمكن تحديد العلاقة',ans:2}, // QB-QC-005
          // ── q5: الإحصاء والاحتمالات ──
          {qnum:21,skill:'q5',text:'سجّل خمسة طلاب الدرجات التالية: 12، 15، 18، 20، 25. فما المتوسط الحسابي لهذه الدرجات؟',opt1:'17',opt2:'18',opt3:'18.5',opt4:'19',ans:1}, // QB-ST-001
          {qnum:22,skill:'q5',text:'رتب القيم التالية تصاعديًا ثم حدد الوسيط: 14، 9، 18، 12، 10.',opt1:'10',opt2:'11',opt3:'12',opt4:'14',ans:2}, // QB-ST-002
          {qnum:23,skill:'q5',text:'في البيانات التالية: 7، 5، 9، 7، 6، 5، 7، 8. ما المنوال؟',opt1:'5',opt2:'6',opt3:'7',opt4:'8',ans:2}, // QB-ST-003
          {qnum:24,skill:'q5',text:'يحتوي صندوق على 5 كرات حمراء و3 كرات زرقاء، وتُسحب كرة واحدة عشوائيًا. ما احتمال أن تكون الكرة المسحوبة زرقاء؟',opt1:'(3/8)',opt2:'(5/8)',opt3:'(1/2)',opt4:'(3/5)',ans:0}, // QB-ST-004
          {qnum:25,skill:'q5',text:'لدى متجر 4 ألوان من القمصان و3 مقاسات لكل لون. إذا اختار شخص قميصًا واحدًا، فكم عدد الاختيارات المختلفة الممكنة؟',opt1:'7',opt2:'10',opt3:'12',opt4:'16',ans:2}, // QB-ST-005
        ];
