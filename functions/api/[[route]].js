...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (resource === 'settings') {
      ...
      // access_requests_enabled which is dev-only per its spec ("يتم
      ...
      if (!sub && method === 'PATCH') {
        ...
        const ALLOWED_KEYS = new Set(['quiz_pass_ratio', 'student_id_prefix', 'access_requests_enabled']);
        if (!ALLOWED_KEYS.has(key)) return err('إعداد غير معروف', 400, CORS);
        if (key === 'access_requests_enabled' && claims.role !== 'dev') return err('هذا الإعداد يُعدَّل من لوحة المطور فقط', 403, CORS);
